...
class BookCommand : TabExecutor {
    ...
    private val myDateTimeFormatter = DateTimeFormatter.ofPattern("yyyy 年 MM 月 dd 日 HH 时")
...
//        )
     ...
     *           如果有第三个参数，则表示目标玩家的名称，用于 OP 玩家给其他玩家看书。
     * @return 总是返回 true，表示操作已经被处理。
     *
     * 此函数首先检查参数数量，如果不满足条件则直接返回 false 或 true。
     * 然后根据参数判断是否需要打开书籍，如果是 OP 玩家，还可以指定其他玩家打开书籍。
     */
     ...
     * @return 如果成功导入书籍，则返回 true；如果文件名不正确或书籍已存在，则返回 false。
     *
     * 此函数首先检查数组 p3 的长度是否为 1，如果是，则表示没有提供正确的文件名，因此直接返回 false。
     * 接着，它检查书籍文件名是否存在于 bookFiles 中，如果不存在，表示该书籍不需要导入，直接返回 true。
     * 如果文件名存在，那么它会尝试导入书籍。书籍文件路径是基于插件名称和提供的文件名构建的。
     * 成功导入书籍后，它会将书籍添加到玩家的库存中，并通过发送消息通知玩家导入成功。
     * 最后，函数返回 true，表示书籍成功导入。
     */
     ...
     * 导出的文件名基于玩家名称、当前时间以及书本的标题生成，存储在 plugins 目录下。
     *
     * @param p0 当前玩家对象，必须持有书本才能导出。
     * @param p3 一个字符串数组，其中 p3[1] 被用作导出文件名的一部分。
     * @return 总是返回 true，表示导出操作已启动（尽管实际成功与否取决于后续逻辑）。
     */
     ...
     * @param p3 帮助命令的参数数组，其中 p3[1] 用于指定特定的命令获取详细帮助。
     * @return 总是返回 true，表示帮助信息已成功发送给玩家。
     */
